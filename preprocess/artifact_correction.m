...
%% quality controlling
ac_X = finall_out_X; ac_Y = finall_out_Y; 

Eng_noise = (sum(sum((raw_ac_X-ac_X).^2))+sum(sum((raw_ac_Y-ac_Y).^2)))/...
    (length(raw_ac_X(:))+length(raw_ac_Y(:)));
Eng_all = (sum(sum(raw_ac_X.^2))+sum(sum(raw_ac_Y.^2)))/...
    (length(raw_ac_X(:))+length(raw_ac_Y(:)));
Eng_effect = (sum(sum(ac_X.^2))+sum(sum(ac_Y.^2)))/(length(raw_ac_X(:))+length(raw_ac_Y(:)));
