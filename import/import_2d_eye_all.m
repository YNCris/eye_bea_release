function import_2d_eye_all(data_2d_name)
...
BeA.RawData.X = tempdata.data(:,1);
BeA.RawData.Y = tempdata.data(:,2);
BeA.RawData.D = tempdata.data(:,3);

%% import skeleton
BeA.DataInfo.Skl = cell(size(BeA.RawData.X,2),1);
BeA.DataInfo.Skl{1,1} = 'eye';
BeA.DataInfo.Skl{2,1} = 'eye';
BeA.DataInfo.Skl{3,1} = 'eye';
